...
import { ArrowLeft, Save, Upload, Plus, Trash2, Box, Truck } from "lucide-react"
...
export default function ProductBuilderPage() {
    ...
    // Initial setup with empty channels, will be populated via API
        ...
        const fetchShops = async () => {
            try {
                ...
                if (res.ok) {
                    ...
                    // Initialize channels with available shops
    ...
    useEffect(() => {
        ...
        // Simple generator logic: When tiers change, we might want to re-gen or sync variants
        // For this Demo, let's just make a button "Cập nhật danh sách biến thể" to avoid infinite loops or complexity
    }, [hasVariants, tiers])
            ...
            t1.options.forEach((opt1, idx1) => {
                if (t2 && t2.options.length > 0) {
                    t2.options.forEach((opt2, idx2) => {
                        ...
                        // Try to find existing variant by indices to preserve data
        ...
        <div className="flex flex-col h-full bg-muted/30">
            <div className="sticky top-0 z-10 flex items-center justify-between border-b bg-background px-6 py-4 shadow-sm">
                <div className="flex items-center gap-4">
                    ...
                    <h1 className="text-lg font-semibold">Tạo sản phẩm</h1>
                </div>
                                                    ...
                                                    <FormField control={form.control} name="descriptionHtml" render={({ field }) => (
                                                        <FormItem>
                                                            ...
                                                            <FormDescription className="text-xs">
                                                                Code HTML sẽ được sử dụng khi đăng lên website hoặc sàn hỗ trợ HTML.
                                                            </FormDescription>
                                                            <FormMessage />
                                                        </FormItem>
                                                    )} />
                                ...
                                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                                    <Card>
                                        <CardHeader className="flex flex-row items-center space-y-0 pb-2 gap-2">
                                            ...
                                            <CardTitle className="text-base">Vận chuyển (Logistics)</CardTitle>
                                        </CardHeader>
                                        <CardContent className="space-y-4 pt-4">
                                            <div className="grid grid-cols-2 gap-4">
                                                ...
                                                <FormField control={form.control} name="daysToShip" render={({ field }) => (
                                                    <FormItem><FormLabel>Chuẩn bị hàng (ngày)</FormLabel><FormControl><Input type="number" {...field} /></FormControl>
                                                        <FormDescription className="text-xs">Để 2 ngày nếu hàng có sẵn</FormDescription>
                                    ...
                                    <Card>
                                        <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
                                            <CardTitle className="text-base">Thuộc tính (Attributes)</CardTitle>
                                            ...
                                            {attrFields.map((field, index) => (
                                                <div key={field.id} className="flex gap-2">
                                                    <FormField control={form.control} name={`attributes.${index}.name`} render={({ field }) => (
                                                        <Input placeholder="Tên (vd: Chất liệu)" className="flex-1" {...field} />
                                                    )} />
                                                    <FormField control={form.control} name={`attributes.${index}.value`} render={({ field }) => (
                                                        <Input placeholder="Giá trị (vd: Cotton)" className="flex-1" {...field} />
                                                    )} />
                                        ...
                                        {!hasVariants ? (
                                            <div className="grid grid-cols-2 gap-4">
                                                ...
                                                <FormField control={form.control} name="barcode" render={({ field }) => (
                                                    <FormItem><FormLabel>Barcode (GTIN)</FormLabel><FormControl><Input {...field} /></FormControl><FormMessage /></FormItem>
                                                )} />
                                                <FormField control={form.control} name="price" render={({ field }) => (
                                                    <FormItem><FormLabel>Giá bán (VND)</FormLabel><FormControl><Input type="number" {...field} /></FormControl><FormMessage /></FormItem>
                                                )} />
                                            ...
                                            <div className="space-y-6">
                                                ...
                                                <div className="bg-muted/30 p-4 rounded-lg space-y-4 border">
                                                    ...
                                                    {tierFields.map((tier, tierIndex) => (
                                                        <div key={tier.id} className="space-y-2">
                                                            ...
                                                            <div className="pl-4 border-l-2 ml-2">
                                                                <p className="text-xs text-muted-foreground mb-2">Nhập các tùy chọn, cách nhau bởi dấu phẩy (vd: Đỏ, Xanh):</p>
                                                                <Input
                                                    ...
                                                    <Button type="button" className="w-full mt-2" onClick={generateVariants}>
                                                        Tạo danh sách biến thể (Variant Matrix)
                                                    </Button>
                                                </div>
                                                                    ...
                                                                    <TableRow key={index}>
                                                                        ...
                                                                        <TableCell>
                                                                            <Input
                                                                                ...
                                                                                onChange={(e) => form.setValue(`variants.${index}.barcode`, e.target.value)}
                                                                                className="h-8"
                                                                                placeholder="Barcode"
                                                                            />
                                                                        </TableCell>
