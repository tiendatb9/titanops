...
export const ShopeeClient = {
    ...
    async getAttributes(accessToken: string, shopId: number, categoryId: number) {
        ...
        const url = `https://partner.shopeemobile.com${path}?partner_id=${PARTNER_ID}&timestamp=${timestamp}&access_token=${accessToken}&shop_id=${shopId}&sign=${sign}&category_id=${categoryId}&language=vi`

        const res = await fetch(url, { method: "GET" })
        if (!res.ok) {
            const err = await res.text()
            throw new Error(`Shopee GetAttributes Error: ${err}`)
        }
        return res.json()
    }
}
